...
func VerifyHttp(pr string) bool {
	return VerifyProxy2(pr, "http")
}
func VerifyProxy2(pr string, pr_type string) bool {
	proxyUrl, proxyErr := url.Parse(pr_type + "://" + pr)
	if proxyErr != nil {
		return false
	}
	tr := http.Transport{
	...
	if hasStr := strings.Contains(result, "百度一下，你就知道"); hasStr {
		return true
	}
	return false
}
func VerifyHttps(pr string) bool {
	return VerifyProxy2(pr, "https")
	...
	if hasStr := strings.Contains(string(bytes[:read]), "200 Connection established"); hasStr {
		return true
	}
	return false
}
